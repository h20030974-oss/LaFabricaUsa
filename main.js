...
function updateQty(val) {
    ...
    if (next >= 1 && next <= 10) {
        currentQty = next;
        document.getElementById('qtyVal').innerText = currentQty;
        // زيادة الثمن مع الكمية في المودال
...
function updateCartUI() {
    ...
    cartItems.innerHTML = cart.map((item, index) => `
        <div style="display:flex; gap:10px; margin-bottom:15px; background:#111; padding:10px; border-radius:4px;">
            <img src="${item.img}" style="width:50px; height:50px; object-fit:cover">
            <div style="flex:1">
                <h4 style="font-size:12px">${item.name}</h4>
                <p style="color:var(--accent); font-size:14px">${item.price * item.qty} MAD (x${item.qty})</p>
            </div>
            <i class="fas fa-trash" onclick="removeFromCart(${index})" style="cursor:pointer; color:red"></i>
        </div>
    `).join('');
...
function sendOrder() {
    ...
    const total = cart.reduce((sum, item) => sum + (item.price * item.qty), 0);
    msg += `%0A*المجموع:* ${total} MAD%0A%0A*الزبون:* ${name}%0A*المدينة:* ${city}`;
